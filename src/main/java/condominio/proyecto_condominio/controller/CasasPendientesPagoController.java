...
public class CasasPendientesPagoController implements Initializable {
    ...
    private void inicializarFiltros() {
        ...
        choiceBoxCPPFiltroAno.setValue(anioActual);
        actualizarMesesDisponibles(anioActual);

        choiceBoxCPPFiltroAno.getSelectionModel().selectedItemProperty().addListener((obs, oldV, newV) -> {
            if (newV != null) {
        ...
        choiceBoxCPPFiltroMes.getSelectionModel().selectedItemProperty().addListener((obs, oldV, newV) -> {
            if (newV != null) {
                cargarDatos();
            }
        });
    ...
    private void actualizarMesesDisponibles(int anioSeleccionado) {
        ...
        String[] meses = logic.getNombresMeses();

        for (int i = mesInicio; i <= mesFin; i++) {
            choiceBoxCPPFiltroMes.getItems().add(meses[i - 1]);
        }

        if (mesPrevio != null && choiceBoxCPPFiltroMes.getItems().contains(mesPrevio)) {
            choiceBoxCPPFiltroMes.setValue(mesPrevio);
        } else {
            choiceBoxCPPFiltroMes.setValue(meses[LocalDate.now().getMonthValue() - 1]);
        }
    }
    ...
    private void cargarDatos() {
        ...
        String mes = choiceBoxCPPFiltroMes.getValue();
        int anio = choiceBoxCPPFiltroAno.getValue();

        var datos = logic.obtenerDatos(mes, anio, limitesGlobales);

        if (mes.equals("Ver todo")) {
            lblCPPFecha.setText("Pendientes del año " + anio);
        } else {
            lblCPPFecha.setText("Cuota de " + mes + " del " + anio);
        }
