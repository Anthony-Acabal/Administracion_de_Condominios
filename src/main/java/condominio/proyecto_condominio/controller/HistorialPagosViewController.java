...
public class HistorialPagosViewController implements Initializable {
    ...
    private HistorialPagoLogic historialLogic = new HistorialPagoLogic();

    @FXML
    private void regresarRegistroPago() {
        try {
            InicioController.getController().cargarVistaEnCentro("/condominio/proyecto_condominio/ui/RegistroPagoView.fxml");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        colFecha.setCellFactory(columna -> new TableCell<HistorialPago, String>() {
            @Override
        ...
        colMonto.setCellFactory(columna -> new TableCell<HistorialPago, Double>() {
            @Override
            protected void updateItem(Double monto, boolean empty) {
                ...
                if (empty || monto == null) {
                    ...
                    setText("Q" + String.format("%,.2f", monto));
                    setStyle("-fx-alignment: CENTER-RIGHT;");
                }
            }
        });
        ...
        colComprobante.setCellFactory(columna -> new TableCell<HistorialPago, String>() {
...

            {
                btnImprimir.setOnAction(event -> {
                    HistorialPago pago = getTableView().getItems().get(getIndex());
                });
            }
        ...
        Label lblPlaceholder = new Label("No hay registros de pagos");
        lblPlaceholder.setStyle("-fx-text-fill: #666666; -fx-font-size: 15px; -fx-font-weight: bold;");
        tblPagos.setPlaceholder(lblPlaceholder);

        cbAnio.setOnAction(event -> cargarMeses());
    }

    private void cargarCombos() {
        cbCasa.getItems().add("Todas las casas");
        cbCasa.getItems().addAll(historialLogic.obtenerCasas());
        cbCasa.getSelectionModel().selectFirst();

        cbAnio.getItems().add("Todos los años");
        int anioActual = LocalDate.now().getYear();
        for (int i = 2026; i <= anioActual; i++) {
            cbAnio.getItems().add(String.valueOf(i));
        }

        cbMes.setValue("Todos los meses");
        cbAnio.setValue("Todos los años");
        cargarMeses();
    }
    ...
    private void buscarPagos() {
        ...
        if (!historialLogic.validarMesAnio(mes, anio)) {
            Alert alerta = new Alert(Alert.AlertType.WARNING);
            alerta.setTitle("Filtro incompleto");
            alerta.setHeaderText(null);
            alerta.setContentText("Debe seleccionar un año.");
            alerta.showAndWait();
            return;
        }

        listaPagos.addAll(historialLogic.filtrarPagos(casa, mes, anio));
    ...
    private void cargarMeses() {
        ...
        if (anioSeleccionado.equals("Todos los años")) {
            limiteMes = LocalDate.now().getMonthValue();
        } else if (Integer.parseInt(anioSeleccionado) == anioActual) {
            limiteMes = LocalDate.now().getMonthValue();
        } else {
            limiteMes = 12;
        }

        for (int i = 0; i < limiteMes; i++) {
            cbMes.getItems().add(meses[i]);
        }

        if (mesSeleccionado != null && cbMes.getItems().contains(mesSeleccionado)) {
            cbMes.setValue(mesSeleccionado);
        } else {
            cbMes.setValue("Todos los meses");
        }
    }
