...
public class ReporteGeneralController implements Initializable {
    ...
    private ReporteGeneralLogic logic = new ReporteGeneralLogic();
    ...
    private void cargarDatos() {
        ...
        List<ReporteGeneral> datos = logic.obtenerReporteGeneral(mesSeleccionado, anioSeleccionado);

        listaReporte = FXCollections.observableArrayList(datos);
        tbReporteGeneral.setItems(listaReporte);

        double[] resumenMes = logic.obtenerResumenMensual(mesSeleccionado, anioSeleccionado);
        double recaudadoMes = resumenMes[0];
        double esperadoMes = resumenMes[1];
        double pendienteMes = Math.max(0, esperadoMes - recaudadoMes);

        lblTotalRecaudadoMes.setText(String.format("Total recaudado de %s %d: Q%.2f / esperado: Q%.2f", mesNombre, anioSeleccionado, recaudadoMes, esperadoMes));
        actualizarGrafica(charGraficaReporteGeneralMes, recaudadoMes, pendienteMes, "#2ecc71", "#9b59b6");

        double[] resumenAno = logic.obtenerResumenAnual(mesSeleccionado, anioSeleccionado);
        double recaudadoAno = resumenAno[0];
        double esperadoAno = resumenAno[1];
        double pendienteAno = Math.max(0, esperadoAno - recaudadoAno);

        lblTotalRecaudadoAno.setText(String.format("Total recaudado del año %d (acumulado a %s): Q%.2f / esperado anual: Q%.2f", anioSeleccionado, mesNombre, recaudadoAno, esperadoAno));
    ...
    private void actualizarGrafica(PieChart grafica, double recaudado, double pendiente, String colorRecaudado, String colorPendiente) {
        ...
        PieChart.Data sliceRecaudado = new PieChart.Data(String.format("Recaudado: Q%.2f (%.1f%%)", recaudado, porcRecaudado), recaudado);
        pieChartData.add(sliceRecaudado);

        if (pendiente > 0) {
            PieChart.Data slicePendiente = new PieChart.Data(String.format("Pendiente: Q%.2f (%.1f%%)", pendiente, porcPendiente), pendiente);
