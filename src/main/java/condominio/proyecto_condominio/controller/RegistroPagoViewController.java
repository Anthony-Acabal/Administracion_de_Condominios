...
public class RegistroPagoViewController implements Initializable {
    ...
    private void abrirHistorial(ActionEvent event) {
        try {
            InicioController mainController = InicioController.getController();
            if (mainController != null) {
                mainController.cargarVistaEnCentro("/condominio/proyecto_condominio/ui/HistorialPagosView.fxml");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void registrarPago(ActionEvent event) {
        ...
        String validacion = registroLogic.validarPago(numeroCasa, mesSeleccionado, anioSeleccionado);
        ...
        boolean registrado = registroLogic.registrarPago(numeroCasa, mesSeleccionado, anioSeleccionado);
    ...
    private void cargarCasas() {
        cbCasa.getItems().addAll(registroLogic.obtenerCasas());
    }
    ...
    private void cargarMesesPendientes() {
        ...
        cbMes.getItems().addAll(registroLogic.obtenerMesesPendientes(numeroCasa, anio));
    ...
    private void cerrarVentana(ActionEvent event) {
        cbCasa.getSelectionModel().clearSelection();
        txtNombre.clear();
        txtApellido.clear();
        txtTelefono.clear();
        txtCuota.clear();
        cbMes.getItems().clear();
        cbAnio.setValue(String.valueOf(LocalDate.now().getYear()));
    }
